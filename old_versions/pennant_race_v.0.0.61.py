...
# ALSO looks like we have scenarios where (starting) pitcher allowed only unearned runs and but was either getting a win, or they other pitcher would get a loss
...
log_filename = f"C:\\Users\\vadim\\Documents\\Code\\_pennant_race\\logs\\pennant_race_log_{timestamp}.txt"
...
BASE_DIRECTORY = 'C:\\Users\\vadim\\Documents\\Code\\_pennant_race\\'
...
class Game:
    ...
    def adjust_bv_for_relievers(self, team, relief_pitching, opponent_pitcher, reliever_innings_distribution, chosen_relievers):
        """
        Adjust the team's batting value based on the handedness of the chosen relievers.
        """
        total_modifier = 0  # Will accumulate the net effect of all relievers
        total_innings_pitched = 0  # Track the total innings pitched by the relievers
        chosen_relievers = relief_pitching.used_relievers
        ...
        for reliever in chosen_relievers:
            ...
            # Loop over players in the lineup and apply split modifiers
            for player in team.players:
                if player.role == 'Starter':
                    ...
                    logger.debug(f"Line 410 Processing {player.name} (Bats: {player.bats}) vs. Reliever {reliever.name} (Throws: {reliever_throws})")
                    ...
                    if reliever_throws == 'L':
                        if player.bats == 'L':
                            ...
                            batter_modifier = 0  # Batter's split against switch-hitters
                            reliever_modifier = 0  # Reliever's split against switch-hitters
                        ...
                        if player.bats == 'L':
                            ...
                            batter_modifier = 0  # Batter's split against switch-hitters
                            reliever_modifier = 0  # Reliever's split against switch-hittersHB
                    ...
                    # Scale the impact of the reliever based on innings pitched
                    total_modifier += (adjusted_player_bv) * (innings_pitched / 9.0)

                    logger.debug(f"LINE 441 {player.name} BV: {player_bv} / modifier {batter_modifier} vs {reliever.name} reliever modifier: {reliever_modifier}, adjusted BV: {adjusted_player_bv}")

        # Adjust BV based on the cumulative effect of all relievers
        if total_innings_pitched > 0:
            team_bv_adjustment = total_modifier / total_innings_pitched

            # We should apply the adjustment directly without recalculating the original BV again
            logger.debug(f"Line 451 Adjusted BV after relievers: {team_bv_adjustment}")
            return team_bv_adjustment
        
        # Return 0 adjustment if no innings were pitched by relievers (keeping the existing BV from starters)
        logger.debug(f"Line 455 No innings pitched by relievers, returning original BV without adjustment")
        return 0  # No adjustment if no innings pitched by relievers
    ...
    def resolve_team_runs(self, team, opponent_pitcher, current_day, is_visiting=True):
        ...
        if not is_complete_game:
            ...
            BV = self.adjust_bv_for_relievers(team, relief_pitching, opponent_pitcher, reliever_innings_distribution, chosen_relievers)
            logger.debug(f"Step 3a: Adjusted Batting Value (BV) for {team.team_name}: {BV}")
            # logger.debug(f"Step 3a: Total relief value after adjustments: {total_relief_value}")
        ...
        for league in ['AL', 'NL']:
            for team_id in self.playoff_teams[league]['division_winners'] + [self.playoff_teams[league]['wildcard']]:
                ...
                for pitcher in team.pitchers:
                    ...
                    logger.debug(f"Initialized pitcher: {pitcher.name}, last_start_day: {pitcher.last_start_day}, rest: {pitcher.rest}")
