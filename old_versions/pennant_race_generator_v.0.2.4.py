...
# need to skew specialists to be mostly LHP (LOOGY)
...
# name file: need chatbot to separate la, del and van surnames eg Delprado should be Del Prado, Vanhorne should be Van Horne etc (found: Jose Jaimesbenitez), also Mc names need third letter capitalized (McGillis)
...
# we seem to be getting a low percentage of dominican players (should be 13-15%)
...
# leadership attribute - this could help mitigate low morale, or increase morale when i get around to making team morale calcs
# i can make indiv morale calc function just to start counting it appropriately, will actually have impact in game when i make GM module
# add manager archetypes to generate manager types (tendencies, ratings etc so that those attributes have some logical connection, like the players already have with their archetypes)
# display overall team grades for offense, defense, starters and relievers (adding up the ratings of the players on the team) and displaying them as grades A-F somewhere on the team page
# 8 rest should only be used for relievers and 0.5 starters
...
log_filename = f"C:\\Users\\vadim\\Documents\\Code\\_pennant_race\\league_files\\_fic_gen_log_{timestamp}.txt"

# Create a logger
logger = logging.getLogger('FictionalGenerator')
...
class NameGen:
    ...
    def generate_bio(self, position=None):
        logger.debug(f"LINE 179 Generating bio for {self.first_name} {self.surname}...")
        ...
        logger.debug(f"LINE 191 - Generated player bio: {self.first_name} {self.surname}, Race: {self.race}, Origin: {self.origin}")
        ...
        logger.debug(f"LINE 209 - Generated school for {self.first_name} {self.surname}: School Name: {self.school}, School Type: {self.school_type}")
    ...
    def generate_height_weight_and_position(self, position=None):
        ...
        logger.debug(f"LINE 317 - Generated position for {self.first_name} {self.surname}: Selected Position {position}")

        return round(height), round(weight), position  # Return the selected position

    def generate_secondary_position(self, position):
        logger.debug(f"LINE 322 - Starting to generate secondary position for {self.first_name} {self.surname}")
        ...
        logger.debug(f"LINE 355 - RNG for {position}: {rng}")

        if rng > chance_of_secondary.get(position, 0.0):
            logger.debug(f"LINE 358 - No secondary generated for {position} because RNG ({rng}) > chance ({chance_of_secondary[position]}).")
            return None

        possible_positions, weights = secondary_position_options.get(position, ([], []))
        logger.debug(f"LINE 362 - Possible positions for {position}: {possible_positions}")

        # Check if there are any possible secondary positions
        if not possible_positions:
            logger.debug(f"LINE 366 - No possible secondary positions for {position}.")
            return None
        ...
        logger.debug(f"LINE 371 - STEP 3 possible positions: {possible_positions} and weights: {weights}")
        logger.debug(f"LINE 372 - Generated secondary position for {position}: {secondary_position}")
        logger.debug(f"LINE 373 - Finished generating secondary position for {self.first_name} {self.surname} / P: {position} / SP: {secondary_position}")
    ...
    def generate_age(self, position):
        ...
        logger.debug(f"LINE 410 - Generated age for {self.first_name} {self.surname} ({position}): {age}")
    ...
    def generate_school_with_type(self, origin):
        ...
        logger.debug(f"LINE 447 - Generated School Name: {school_name}, School Type: {school_type}")
...
class Archetype:
    ...
    def get_archetype_probs(school_name, school_type):
        logger.debug(f"LINE 483 - Entering get_archetype_probs - School: {school_name}, Type: {school_type}")
        
        # Handle international schools directly based on school_type
        if school_type == "INT":
            logger.debug(f"LINE 487 - International school detected. Using baseline probabilities.")
            return {
        ...
        logger.debug(f"LINE 500 - School type: {school_type}, School DataFrame: {school_df}")
        ...
        logger.debug(f"LINE 504 - Matching rows for school '{school_name}' in {school_type}: {matching_rows} - School DataFrame: {school_df}")

        if matching_rows.empty:
            # If no matching school is found, use default probabilities
            logger.debug(f"LINE 508 - Warning: School '{school_name}' not found in {school_type}. Using default probabilities.")
            return {
                'grade1_prob': 0.15, 'grade2_prob': 0.3, 'grade3_prob': 0.4,
                'grade4_prob': 0.5, 'grade5_prob': 0.6, 'grade6_prob': 0.7,
                'grade7_prob': 0.85, 'grade8_prob': 0.95, 'grade9_prob': 0.98,
        ...
        logger.debug(f"LINE 518 - Found school '{school_name}' with probabilities: {school_row[['grade1_prob', 'grade10_prob']]}")
        return {
...
class BatterProfile:
    ...
    batter_archetypes = [
        Archetype("scrub", 
            contact_range=(0, 1), power_range=(0, 1), eye_range=(-3, -2), speed_range=(0, 1), splits_range=(-3, -1), 
            fielding_range=(-3, -2), potential_range=(0, 1), 
            dev_ceiling_age=21, decline_age=28),
        
        Archetype("career minor leaguer", 
            contact_range=(0, 2), power_range=(0, 2), eye_range=(-3, -1), speed_range=(0, 2), splits_range=(-3, 0),
            fielding_range=(-3, 0), potential_range=(0, 2), 
            dev_ceiling_age=22, decline_age=29),
        
        Archetype("september callup", 
            contact_range=(1, 2), power_range=(1, 2), eye_range=(-1, 0), speed_range=(1, 2), splits_range=(-2, 0),
            fielding_range=(-2, 0), potential_range=(0, 3), 
            dev_ceiling_age=23, decline_age=29),
        
        Archetype("injury replacement", 
            contact_range=(2, 3), power_range=(1, 2), eye_range=(-1, 0), speed_range=(1, 3), splits_range=(-2, 1),
            fielding_range=(-2, 0), potential_range=(1, 3), 
            dev_ceiling_age=23, decline_age=29),
        
        Archetype("AAAA player", 
            contact_range=(2, 4), power_range=(1, 3), eye_range=(0, 0), speed_range=(1, 3), splits_range=(-1, 1),
            fielding_range=(-2, 1), potential_range=(2, 4), 
            dev_ceiling_age=24, decline_age=30),
        
        Archetype("backup", 
            contact_range=(2, 5), power_range=(1, 3), eye_range=(0, 1), speed_range=(1, 4), splits_range=(-1, 2),
            fielding_range=(-2, 1), potential_range=(3, 4), 
            dev_ceiling_age=25, decline_age=31),
        
        Archetype("platoon", 
            contact_range=(3, 5), power_range=(1, 5), eye_range=(0, 2), speed_range=(1, 5), splits_range=(-3, 3),
            fielding_range=(-1, 2), potential_range=(3, 5), 
            dev_ceiling_age=25, decline_age=32),
        
        Archetype("regular starter", 
            contact_range=(4, 6), power_range=(2, 6), eye_range=(0, 2), speed_range=(2, 6), splits_range=(0, 2),
            fielding_range=(-1, 3), potential_range=(4, 6), 
            dev_ceiling_age=25, decline_age=33),
        
        Archetype("star", 
            contact_range=(5, 7), power_range=(2, 7), eye_range=(1, 3), speed_range=(2, 7), splits_range=(1, 2),
            fielding_range=(0, 3), potential_range=(5, 7), 
            dev_ceiling_age=25, decline_age=34),
        
        Archetype("5-tool", 
            contact_range=(6, 8), power_range=(4, 8), eye_range=(2, 3), speed_range=(6, 8), splits_range=(1, 3), 
            fielding_range=(1, 3), potential_range=(5, 8), 
            dev_ceiling_age=25, decline_age=35)
    ]

    def generate_ratings(self):
        # Use archetype ranges to generate ratings
        logger.debug(f"LINE 624 - Generating ratings for batter: {self.name}, Archetype: {self.archetype.name}")
        ...
        logger.debug(f"LINE 632 - Ratings generated for {self.name}: Contact: {self.contact}, Power: {self.power}, Eye: {self.eye}, Speed: {self.speed}, Fielding: {self.fielding}, Potential: {self.potential}")
        ...
        logger.debug(f"LINE 639 - Injury rating for {self.name}: {self.injury}")

        self.makeup = max(min(int(np.random.normal(0, 1)), 2), -2)

        self.clutch = max(min(int(np.random.normal(0, 1)), 2), -2)
        logger.debug(f"LINE 644 - Makeup: {self.makeup}, Clutch: {self.clutch}")
                      
        # Adjust ratings by age and log any changes
        logger.debug(f"LINE 647 - Adjusting ratings by age for {self.name}, Age: {self.age}")
        self.adjust_by_age()

        logger.debug(f"LINE 650 - Final adjusted ratings for {self.name}: Contact: {self.contact}, Power: {self.power}, Speed: {self.speed}, Fielding: {self.fielding}")
    ...
    def generate_splits(self):
        logger.debug(f"LINE 685 - Generating splits for {self.name}")
    ...
    def generate_attribute(self, range_tuple, attribute_name, age_affected=True):
        logger.info(f"LINE 807 Generating {attribute_name} for {self.name}...")
    ...
    def adjust_by_age(self):
        ...
        logger.debug(f"LINE 854 - Development difference: {dev_diff}, Decline difference: {decline_diff}")
        ...
        if self.bio['age'] < self.archetype.dev_ceiling_age:
            logger.debug(f"LINE 869 - {self.name} is under development ceiling. Reducing ratings by {dev_diff}")
            ...
            logger.debug(f"LINE 876 - {self.name} is past decline age. Reducing ratings by {decline_diff}")
        ...
        logger.debug(f"LINE 890 - Adjusted ratings for {self.name}: Contact: {self.contact}, Power: {self.power}, Speed: {self.speed}, Fielding: {self.fielding}")
...
class PitcherProfile:
    ...
    starter_archetypes = [
        Archetype("journeyman", 
            start_value_range=(0.5, 1.0), endurance_range=(0.5, 1.0), rest_range=(6, 8), splits_range=(-3, -1),
            cg_rating_range=(666, 666), sho_rating_range=(666, 666), relief_value_range=(0.5, 1.0), 
            fatigue_range=(1.0, 2.0), potential_range=(0, 0), dev_ceiling_age=21, decline_age=28),

        Archetype("fringe starter", 
            start_value_range=(1.0, 2.0), endurance_range=(1.0, 2.0), rest_range=(5, 8), splits_range=(-3, 0),
            cg_rating_range=(665, 666), sho_rating_range=(666, 666), relief_value_range=(0.5, 1.0), 
            fatigue_range=(1.0, 2.0), potential_range=(0, 1), dev_ceiling_age=22, decline_age=29),

        Archetype("late bloomer", 
            start_value_range=(1.0, 2.5), endurance_range=(1.0, 2.5), rest_range=(5, 7), splits_range=(-2, 0),
            cg_rating_range=(664, 666), sho_rating_range=(666, 666), relief_value_range=(0.5, 1.0), 
            fatigue_range=(1.0, 2.0), potential_range=(0, 1), dev_ceiling_age=23, decline_age=29),

        Archetype("spot starter", 
            start_value_range=(1.5, 2.5), endurance_range=(1.0, 3.0), rest_range=(4, 7), splits_range=(-2, 1),
            cg_rating_range=(666, 666), sho_rating_range=(666, 666), relief_value_range=(0.5, 1.0), 
            fatigue_range=(1.0, 2.0), potential_range=(0, 1), dev_ceiling_age=23, decline_age=29),

        Archetype("quad-a arm", 
            start_value_range=(1.5, 3.0), endurance_range=(1.5, 3.0), rest_range=(4, 6), splits_range=(-1, 0),
            cg_rating_range=(665, 665), sho_rating_range=(665, 665), relief_value_range=(0.5, 1.0), 
            fatigue_range=(1.0, 2.0), potential_range=(0, 2), dev_ceiling_age=24, decline_age=30),

        Archetype("swingman", 
            start_value_range=(1.5, 3.5), endurance_range=(2.0, 4.0), rest_range=(4, 6), splits_range=(-1, 1),
            cg_rating_range=(665, 665), sho_rating_range=(665, 665), relief_value_range=(1.0, 2.5), 
            fatigue_range=(2.0, 3.0), potential_range=(0, 3), dev_ceiling_age=25, decline_age=31),

        Archetype("back of rotation", 
            start_value_range=(2.0, 4.5), endurance_range=(3.0, 5.0), rest_range=(4, 6), splits_range=(-1, 2),
            cg_rating_range=(661, 666), sho_rating_range=(661, 661), relief_value_range=(0.5, 1.0), 
            fatigue_range=(1.0, 2.0), potential_range=(1, 3), dev_ceiling_age=25, decline_age=32),

        Archetype("regular starter", 
            start_value_range=(4.0, 6.0), endurance_range=(3.0, 6.0), rest_range=(4, 5), splits_range=(0, 2),
            cg_rating_range=(661, 666), sho_rating_range=(656, 666), relief_value_range=(0.5, 1.0), 
            fatigue_range=(1.0, 2.0), potential_range=(1, 5), dev_ceiling_age=25, decline_age=33),

        Archetype("top of rotation", 
            start_value_range=(5.0, 7.0), endurance_range=(4.0, 6.5), rest_range=(4, 5), splits_range=(1, 2),
            cg_rating_range=(641, 666), sho_rating_range=(651, 666), relief_value_range=(0.5, 1.0), 
            fatigue_range=(1.0, 2.0), potential_range=(3, 7), dev_ceiling_age=25, decline_age=34),

        Archetype("ace", 
            start_value_range=(6.0, 8.0), endurance_range=(5.0, 8.0), rest_range=(4, 4), splits_range=(1, 3),
            cg_rating_range=(611, 666), sho_rating_range=(611, 666), relief_value_range=(0.5, 1.0), 
            fatigue_range=(1.0, 2.0), potential_range=(4, 8), dev_ceiling_age=25, decline_age=35)
    ]

    reliever_archetypes = [
        Archetype("filler arm", 
            start_value_range=(0.5, 1.0), endurance_range=(0.5, 1.0), rest_range=(8, 8), splits_range=(-3, -1),
            cg_rating_range=(666, 666), sho_rating_range=(666, 666), relief_value_range=(0.5, 1.0), 
            fatigue_range=(0.5, 1.0), potential_range=(0, 0), dev_ceiling_age=21, decline_age=28),

        Archetype("taxi squad reliever", 
            start_value_range=(0.5, 1.0), endurance_range=(0.5, 1.0), rest_range=(7, 8), splits_range=(-2, -1),
            cg_rating_range=(665, 666), sho_rating_range=(666, 666), relief_value_range=(1.0, 2.0), 
            fatigue_range=(1.0, 2.0), potential_range=(0, 1), dev_ceiling_age=22, decline_age=29),

        Archetype("roster expansion reliever", 
            start_value_range=(0.5, 1.0), endurance_range=(0.5, 1.0), rest_range=(7, 8), splits_range=(-2, 0),
            cg_rating_range=(664, 666), sho_rating_range=(666, 666), relief_value_range=(1.0, 2.5), 
            fatigue_range=(1.0, 2.5), potential_range=(0, 1), dev_ceiling_age=23, decline_age=29),

        Archetype("bullpen patch", 
            start_value_range=(0.5, 1.0), endurance_range=(0.5, 1.0), rest_range=(7, 8), splits_range=(-1, 0),
            cg_rating_range=(666, 666), sho_rating_range=(666, 666), relief_value_range=(1.5, 2.5), 
            fatigue_range=(1.0, 3.0), potential_range=(0, 1), dev_ceiling_age=23, decline_age=29),

        Archetype("perpetual callup", 
            start_value_range=(0.5, 1.0), endurance_range=(0.5, 1.0), rest_range=(7, 8), splits_range=(-1, 1),
            cg_rating_range=(666, 666), sho_rating_range=(666, 666), relief_value_range=(1.5, 3.0), 
            fatigue_range=(1.5, 3.0), potential_range=(0, 2), dev_ceiling_age=24, decline_age=30),

        Archetype("specialist", 
            start_value_range=(0.5, 1.0), endurance_range=(0.5, 1.0), rest_range=(7, 7), splits_range=(-3, 3),
            cg_rating_range=(666, 666), sho_rating_range=(666, 666), relief_value_range=(1.5, 3.5), 
            fatigue_range=(2.0, 4.0), potential_range=(0, 3), dev_ceiling_age=25, decline_age=31),

        Archetype("long-relief", 
            start_value_range=(1.0, 1.5), endurance_range=(1.0, 1.5), rest_range=(5, 7), splits_range=(0, 1),
            cg_rating_range=(665, 666), sho_rating_range=(665, 666), relief_value_range=(2.0, 4.5), 
            fatigue_range=(3.0, 5.0), potential_range=(1, 3), dev_ceiling_age=25, decline_age=32),

        Archetype("low-leverage", 
            start_value_range=(0.5, 1.0), endurance_range=(0.5, 1.0), rest_range=(7, 8), splits_range=(0, 2),
            cg_rating_range=(666, 666), sho_rating_range=(666, 666), relief_value_range=(4.0, 6.0), 
            fatigue_range=(3.0, 6.0), potential_range=(1, 5), dev_ceiling_age=25, decline_age=33),

        Archetype("high-leverage", 
            start_value_range=(0.5, 1.0), endurance_range=(0.5, 1.0), rest_range=(7, 8), splits_range=(1, 2),
            cg_rating_range=(666, 666), sho_rating_range=(666, 666), relief_value_range=(5.0, 7.0), 
            fatigue_range=(4.0, 6.5), potential_range=(3, 7), dev_ceiling_age=25, decline_age=34),

        Archetype("closer", 
            start_value_range=(0.5, 1.0), endurance_range=(0.5, 1.0), rest_range=(7, 8), splits_range=(1, 3),
            cg_rating_range=(666, 666), sho_rating_range=(666, 666), relief_value_range=(6.0, 8.0), 
            fatigue_range=(5.0, 8.0), potential_range=(4, 8), dev_ceiling_age=25, decline_age=35)
    ]

    def generate_ratings(self):
        logger.info(f"LINE 1036 Generating ratings for {self.name}")
        ...
        self.fatigue = self.generate_attribute(self.archetype.fatigue_range, "fatigue")
        self.potential = self.generate_attribute(self.archetype.potential_range, "potential", age_affected=False)

        logger.debug(f"LINE 1047 - Ratings generated for {self.name}: Start Value: {self.start_value}, Endurance: {self.endurance}, Rest: {self.rest}, CG Rating: {self.cg_rating}, SHO Rating: {self.sho_rating}, Relief Value: {self.relief_value}, Fatigue: {self.fatigue}, Potential: {self.potential}")

        # Generate splits for L vs R batters
        self.splits_L, self.splits_R = self.generate_splits()
        logger.debug(f"LINE 1051 - Splits generated for {self.name}: Left: {self.splits_L}, Right: {self.splits_R}")

        # Generate random injury rating
        self.injury = max(min(int(np.random.normal(0, 1.5)), 3), -3)
        logger.debug(f"LINE 1055 - Injury rating for {self.name}: {self.injury}")

        # Adjust ratings by age
        logger.debug(f"LINE 1058 - Adjusting ratings by age for {self.name}, Age: {self.age}")
        self.adjust_by_age()
        
        self.makeup = self.generate_makeup()

        logger.debug(f"LINE 1063 - Final adjusted ratings for {self.name}: Start Value: {self.start_value}, Endurance: {self.endurance}, Rest: {self.rest}, CG Rating: {self.cg_rating}, SHO Rating: {self.sho_rating}, Relief Value: {self.relief_value}, Fatigue: {self.fatigue}, Potential: {self.potential}")
        

    def generate_makeup(self):
        logger.info(f"LINE 1067 Generating makeup for {self.name}")
        ...
        makeup = max(min(int(np.random.normal(0, 1)), 3), -3)
        return makeup
    ...
    def generate_splits(self):
        logger.info(f"LINE 1109 Generating pitching splits for {self.name}")
    ...
    def generate_attribute(self, range_tuple, attribute_name, age_affected=True):
        logger.debug(f"LINE 1284 Generating {attribute_name} for {self.name} with range {range_tuple}")
        """Generate a rating based on a given range, skewed by archetype quality."""
        floor, ceiling = range_tuple
        
        # Ensure that floor is always less than or equal to ceiling
        if floor > ceiling:
            floor, ceiling = ceiling, floor  # Swap the values if reversed
        
        # Skewed distribution depending on archetype quality
        if self.archetype.name in ["ace", "closer", "high-leverage", "5-tool"]:
            # High-quality archetypes skew towards the ceiling
            rating = self.skewed_random(floor, ceiling, skew="high")
        elif self.archetype.name in ["scrub", "journeyman", "filler arm"]:
            # Low-quality archetypes skew towards the floor
            rating = self.skewed_random(floor, ceiling, skew="low")
        else:
            # Neutral or moderate archetypes have a balanced distribution
    ...
    def adjust_by_age(self):
        ...
        self.start_value = np.clip(self.start_value, 0.5, 8.0)
        ...
        self.relief_value = np.clip(self.relief_value, 0.5, 8.0)
        self.fatigue = np.clip(self.fatigue, 0.5, 8.0)
...
class ManagerProfile:
    def __init__(self):
        ...
        self.manager_leadership = 0 # -3 to +3, will affect team morale
        ...
        self.manager_experience = 0 # in years
        self.manager_salary = 0

    '''
    manager_archetypes = [ # placeholder for now
        Archetype("smallball",
            manager_hitting_range=(1, 3), manager_pitching_range=(-3, -1), manager_fielding_range=(1, 3), manager_bench_range=(1, 3), manager_potential_range=(1, 3), manager_experience_range=(1, 3), manager_salary_range=(1, 3))
    ]
    '''
    ...
    def to_dict(self):
        return {
            ...
            "manager_experience": self.manager_experience,
...
class ScoutingReport:
    ...
    def load_templates(self):
        ...
        with open(r'C:\Users\vadim\Documents\Code\_pennant_race\scouting_reports.json', 'r') as f:
            return json.load(f)
...
class Team:
    ...
    def generate_roster(self):
        logger.debug(f"LINE 1786 - Generating roster for team: {self.name}")
        ...
        logger.debug(f"LINE 1805 - Assigning roles for {self.name}. Starter for the first 9, Bench for the next 6")
        
        # Assign roles and generate scouting reports for the starting players
        for i, player in enumerate(roster['Starting 9']):
            if player is not None:  # Ensure player is not None before assignment
                player['role'] = 'Starter'  # Assign "Starter" to the first 9 players
                logger.debug(f"LINE 1808 - {player['name']} assigned role: Starter")
                ...
                logger.debug(f"LINE 1812 - No player found for position {i} in the starting lineup.")
        
        # Assign roles and generate scouting reports for the bench players
        for i, player in enumerate(roster['Bench']):
            if player is not None:  # Ensure player is not None before assignment
                player['role'] = 'Bench'  # Assign "Bench" to the bench players
                logger.debug(f"LINE 1816 - {player['name']} assigned role: Bench")

                # Instantiate the ScoutingReport for the player
                scouting_report = ScoutingReport(player)  # Pass the player object to ScoutingReport
                player['scouting_report'] = scouting_report.generate_report()  # Generate and attach the report
            else:
                logger.debug(f"LINE 1820 - No player found for position {i} in the bench lineup.")
        
        # Generate scouting reports for pitchers
        for pitcher in roster['Pitchers']:
            if pitcher is not None:  # Ensure pitcher is not None before assignment
                ...
                logger.debug(f"LINE 1825 - No pitcher found in pitching staff.")
    ...
    def create_player(self, position):
        logger.debug(f"LINE 1917 - Creating player for position: {position}")
        bio = NameGen().generate_bio(position)

        # Now pass the school_type from the bio
        player_profile = PlayerProfile(position, bio)
        logger.debug(f"LINE 1923 - Generated bio for {player_profile.bio['name']} - Position: {player_profile.bio['position']}, School: {player_profile.bio['school']}, Origin: {player_profile.bio['origin']}")
        ...
        logger.debug(f"LINE 1928 - Player {player_profile.bio['name']} school: {school_name}, School type: {school_type}")
        ...
        logger.debug(f"LINE 1932 - Fetched archetype probabilities for {school_name} (type: {school_type}): {school_probs}")
        
        # Fixed probabilities for top two grades
        fixed_ranges = {
            'grade10_prob': (0.99, 1.0),
            'grade9_prob': (0.97, 0.99)
        }
        
        # Generate random probability
        rand_prob = random.random()
        
        # Check fixed top grades first
        if 0.99 <= rand_prob <= 1.0:
            grade_prob = 'grade10_prob'
        elif 0.97 <= rand_prob < 0.99:
            grade_prob = 'grade9_prob'
        else:
            # Now handle grades 1-8, but exclude grades 9 and 10
            cumulative_probs = {}
            cumulative_sum = 0

            # Only sum grades 1-8, stop before grade9_prob
            for col, prob in school_probs.items():
                if col not in ['grade9_prob', 'grade10_prob']:  # Skip the fixed grades
                    cumulative_sum += prob
                    cumulative_probs[col] = cumulative_sum
                
            # Now map the random number to the appropriate grade
            for col, threshold in cumulative_probs.items():
                if rand_prob <= threshold:
                    grade_prob = col
                    logger.debug(f"LINE 1960 {bio['name']}, RNG:{rand_prob} assigned grade_prob: {grade_prob} for archetype mapping.")
                    break
            else:
                # Fallback in case of any unexpected issue
        ...
        if position == 'SP':  # Starter
            ...
            logger.debug(f"LINE 1969 - {player_profile.bio['name']} (Starter) Archetype: {archetype_name}")
            ...
            logger.debug(f"LINE 1975 - {player_profile.bio['name']} (Reliever) Archetype: {archetype_name}")
            ...
            logger.debug(f"LINE 1986 - {player_profile.bio['name']} (Batter) Archetype: {archetype_name}")
        ...
        logger.debug(f"LINE 1991 - Generating ratings for {player_profile.bio['name']}")
...
class LineupManager:
    def __init__(self, roster):
        ...
        self.used_players = set()  # Keep track of used players
    ...
    def construct_lineup(self):
        ...
        self.lineup = [None] * 9  # Empty 9-player lineup

        # Assign top positions in the lineup (leadoff, 2nd, 3rd, cleanup, etc.)
        logger.debug("LINE 2050 Starting lineup construction")
        
        # 1. Find best players by position based on archetype and key ratings.
        for i, position in enumerate(["LF", "CF", "RF", "SS", "2B", "3B", "1B", "C", "DH"]):
            self.lineup[i] = self.get_best_player_for_position(starters, position)
            
            if self.lineup[i] is not None:
                logger.debug(f"Selected {self.lineup[i]['name']} for position {position} in the lineup.")
            else:
                logger.debug(f"No available player for position {position}. Leaving slot {i} empty.")
        
        return self.lineup

    def get_best_player_for_position(self, players, position):
        """Return the best player for a specific position considering both primary and secondary positions."""
        # Filter out already used players from the pool by their name
        available_players = [p for p in players if p['name'] not in self.used_players]  # Use player names to filter

        # Filter by position (either primary or secondary position matches)
        position_players = [p for p in available_players if p['position'] == position or p.get('secondary_position') == position]

        if not position_players:
            logger.debug(f"No available players for position {position}")
            return None

        # Sort players by archetype priority and ratings
        sorted_position_players = sorted(
            position_players,
            key=lambda p: (
                self.get_archetype_priority(p),
                p.get('batting', 0),
                p.get('power', 0),
                p.get('eye', 0),
                p.get('fielding', 0)
        ...
        best_player = sorted_position_players[0]
        self.used_players.add(best_player['name'])  # Mark the player as used by adding their name to the set
        logger.debug(f"Best player for {position}: {best_player['name']} (Archetype: {best_player['archetype']})")
    ...
    def get_archetype_priority(self, player):
        ...
        # Return priority based on player type (batter, starter, reliever)
        if player_type == 'batter':
...
class League:
    ...
    def generate_league_name(self):
        logger.debug(f"LINE 2202 Generating League Name")
    ...
    def select_nickname(self):
        ...
        logger.debug(f"LINE 2224 Selecting Team Nickname")
    ...
    def generate_team_name(self, city_name):
        ...
        logger.debug(f"LINE 2234 Generating Team Name using city name and nickname")
    ...
    def get_city_weight(self, row):
        ...
        # logger.debug(f"LINE 2240 Calculating City Weight")
        population_weight = row['population'] / self.msa_data['population'].max()
        gdp_weight = row['gdp_rank']  # Inverse of the rank, lower rank is better
        return population_weight * gdp_weight

    def generate_league(self):
        teams = []
        logger.debug(f"LINE 2247 Generating League")
        ...
        for team_id in range(1, self.num_teams + 1):
            try:
                ...
                logger.debug(f"LINE 2253 - DEBUG: Error selecting city: {e}")
            ...
            logger.debug(f"LINE 2258 Generating Team Name")
    ...
    def select_city(self, existing_teams):
        ...
        logger.debug(f"LINE 2268 Selecting City")
        # Copy the MSA data
    ...
    def get_coordinates(self, city_name):
        logger.debug(f"LINE 2311 Get Coordinates for {city_name}")
    ...
    def generate_divisions(self):
        logger.debug(f"LINE 2316 Generating Divisions")
    ...
    def save_teams_to_json(self):
        ...
        for team in self.teams:
            ...
            logger.debug(f"LINE 2350 - Saved team {team.city} {team.name} to {file_path}")
    ...
    def print_teams_and_rosters(self):
        ...
        logger.debug(f"LINE 2390 - Rosters saved to {file_path}")

class PlayerProfile:
    logger.debug(f"Line 2393 - PlayerProfile - Initializing PlayerProfile")
    """PlayerProfile class for batter and pitcher profiles."""
    def __init__(self, position, bio):
        self.position = position
        self.bio = bio
        self.school_type = self.bio.get('school_type')  # Directly use the generated school_type
        logger.debug(f"Line 2400 - PlayerProfile - Position: {self.position}, School Type: {self.school_type}")

    def generate_ratings(self):
        """Generate player ratings based on position."""
        logger.debug(f"Line 2404 - Generating player ratings based on position")
    ...
    def to_dict(self):
        ...
        logger.debug(f"Line 2414 - Converting PlayerProfile to dictionary")
