...
class BatterProfile:
    ...
    def __init__(self, bio, archetype):
        ...
        BatterProfile._player_counter += 1
        ...
        self.x_hr = 0  # Extra-base hit rating (calculated from power)
        self.experience = 0  # Years of experience
    ...
    def _generate_player_id(self):
        """Generate a unique player_id based on name (similar to baseball-reference format)."""
        # Format: first 5 chars of surname + first 2 of first name + counter
        name_parts = self.name.split()
        if len(name_parts) >= 2:
            surname = name_parts[-1].lower()[:5]
            firstname = name_parts[0].lower()[:2]
        else:
            surname = self.name.lower()[:5]
            firstname = "xx"
    ...
    def generate_ratings(self):
        ...
        # Calculate x_hr (extra-base hit rating) based on power
        # x_hr ranges from 0-8 based on power rating
        self.x_hr = max(0, min(8, self.power))

        # Calculate experience based on age (rough estimate: years since age 22)
        self.experience = max(0, self.age - 22)
...
class PitcherProfile:
    ...
    def __init__(self, bio, archetype):
        ...
        # Generate unique player_id
        PitcherProfile._pitcher_counter += 1
        self.player_id = self._generate_player_id()
...
class Team:
    ...
    def __init__(self, team_id, name, city, state, league_name, division_name, city_abbrev=None, league_year=2024):
        # Use city_abbrev for string team_id format (e.g., "NYC_2024") to match MLB import format
        self.city_abbrev = city_abbrev or city[:3].upper()
        self.league_year = league_year
        self.team_id = f"{self.city_abbrev}_{self.league_year}"  # String format like MLB
        self.numeric_id = team_id  # Keep numeric id for schedule compatibility
        self.name = name
        self.city = city
        self.state = state
    ...
    def generate_ballpark_name(self):
        """Generate a ballpark name using corporate names and venue types."""
        venue_types = ["Field", "Stadium", "Park", "Ballpark", "Arena", "Coliseum"]
        corporate_name = random.choice(corporate_names)
        venue_type = random.choice(venue_types)
        return f"{corporate_name} {venue_type}"
...
class League:
    ...
    def get_city_weight(self, row):
        """Calculate a combined weight for a city based on its population and GDP."""
        population_weight = row['population'] / self.msa_data['population'].max()
        gdp_weight = row['gdp']  # GDP is already normalized 0-1, higher is better
        return population_weight * gdp_weight

    def generate_league(self, league_year=2024):
        """Generate teams with MLB-style league structure: American/National League, East/Central/West divisions."""
        teams = []
        team_id = 0
        max_attempts = self.num_teams * 3  # Allow retries
        attempts = 0

        # MLB structure: 2 leagues, 3 divisions each, 5 teams per division = 30 teams
        league_structure = [
        ...
        while len(teams) < self.num_teams and attempts < max_attempts:
            attempts += 1
            try:
                city, city_abbrev, state = self.select_city(teams)
            except ValueError as e:
                logger.debug(f"LINE 1517 - DEBUG: Error selecting city (attempt {attempts}): {e}")
                continue  # Retry city selection

            team_id += 1

            # Determine league and division based on team position
            team_index = len(teams)
            league_index = team_index // 15  # 0 for first 15 (AL), 1 for next 15 (NL)
            division_index = (team_index % 15) // teams_per_division  # 0=East, 1=Central, 2=West

            league_name = league_structure[league_index][0]
            division_name = league_structure[league_index][1][division_index]

            # Generate team name using only the nickname, city used separately
            team_name = self.generate_team_name(city)

            # Pass the generated team_id, league name, city_abbrev, and league_year when creating the team
            new_team = Team(
                team_id=team_id,
                name=team_name,
                city=city,
            ...
            logger.debug(f"Created team {len(teams)}/{self.num_teams}: {city} {team_name} ({city_abbrev}_{league_year}) - {league_name} {division_name}")

        if len(teams) < self.num_teams:
            logger.warning(f"Only generated {len(teams)} teams out of {self.num_teams} requested after {attempts} attempts")
    ...
    def select_city(self, existing_teams):
        ...
        Returns tuple of (city_name, city_abbrev, state)."""
        # Copy the MSA data
        ...
        # Count how many teams are in each MSA based on selected_cities tracking
        msas_to_exclude = [msa for msa, cities in self.selected_cities.items() if len(cities) >= 2]
        ...
        if valid_cities.empty:
            raise ValueError("No valid cities remaining - all MSAs at capacity.")

        # Calculate weights for remaining cities
        valid_cities = valid_cities.copy()  # Avoid SettingWithCopyWarning
        valid_cities['weight'] = valid_cities.apply(self.get_city_weight, axis=1)

        # Try up to 10 times to find a valid city within valid MSAs
        for _ in range(10):
            # Select a city randomly, weighted by population and GDP
            city_row = valid_cities.sample(n=1, weights=valid_cities['weight']).iloc[0]
            msa_base = city_row['msa_base']

            # Get city_abbrev from the data (or generate from city name if not available)
            city_abbrev = city_row.get('city_abbrev', city_row['msa'][:3].upper())
            state = city_row.get('state', '')

            # Handle the case where multiple cities are in the same MSA
            msa_cities = city_row['msa'].split('-')
            for city_name in msa_cities:
                city_name = city_name.strip()
                if city_name not in self.selected_cities.get(msa_base, []):
                    # If this city is not already selected, use it
                    self.selected_cities.setdefault(msa_base, []).append(city_name)
                    return city_name, city_abbrev, state

            # If we get here, all cities in this MSA are used - exclude it and retry
            valid_cities = valid_cities[valid_cities['msa_base'] != msa_base]
            if valid_cities.empty:
                raise ValueError("No valid cities remaining after exclusions.")

        raise ValueError("Unable to select a valid city after multiple attempts.")
    ...
    def save_teams_to_json(self):
        ...
        # Use centralized folder path for fictional teams (json_fic directory)
        folder_path = PENNANT_FEVER_JSON_FIC_DIR
        ...
        for team in self.teams:
            ...
            # Use string team_id format matching MLB import (e.g., team_id_NYC_2024.json)
    ...
    def save_league_json(self, league_year=2024):
        """Save league structure to league_fictional.json for game engine compatibility."""
        # Build league structure matching MLB format
        league_structure = {
            "league_id": league_year,
            "league_name": f"Fictional Baseball League {league_year}",
            "schedule_name": "mlb_schedule_2023.txt",
            "sub_leagues": []
        }
        ...
        for team in self.teams:
            ...
            if division_name not in leagues_dict[league_name]:
                leagues_dict[league_name][division_name] = []
            leagues_dict[league_name][division_name].append(team.team_id)

        # Build sub_leagues array
        for league_name in ["American League", "National League"]:
            ...
            for division_name in ["East", "Central", "West"]:
                if division_name in leagues_dict[league_name]:
                    sub_league["divisions"].append({
                        "division_name": division_name,
                        "teams": leagues_dict[league_name][division_name]
                    })
            league_structure["sub_leagues"].append(sub_league)

        # Save to league_fictional.json in the pennant_fever directory
        from data_paths_pennant_fever import PENNANT_FEVER_DIR
        file_path = PENNANT_FEVER_DIR / "league_fictional.json"
        with open(file_path, 'w') as f:
            json.dump(league_structure, f, indent=4)
        logger.info(f"Saved league structure to {file_path}")
...
league.save_league_json()  # Export league structure for game engine
