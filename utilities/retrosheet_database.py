...
def get_player_stats(player_id, year, db_conn):
    ...
    # Modify the query to include PIT_HAND_CD for splits
    query = """
    ...
    for event in events:
        if len(event) == 10:  # Ensure all fields are present
...
def compile_team_stats(ros_files, db_conn):
    ...
    for ros_file in ros_files:
        ros_file_path = os.path.join(roster_dir, ros_file)  # Ensure full path is used
        team_roster = parse_ros_file(ros_file_path)
        for player in team_roster:
            year = ros_file[-8:-4]  # Extract year from filename (e.g., 'TOR1984')
...
# Example usage
ros_files = ["TOR1984.ROS"]  # Use one file for testing
...
save_to_csv(compiled_data, 'retrosheet_db_test.csv')
