...
def calculate_speed_score(data, year):
    ...
    weights = {
        ...
        'f2': 0.25, # Stolen base attempts
        'f3': 0.15, # Triples
        'f4': 0.1,  # Runs scored
        'f5': 0.05, # GIDP
        'f6': 0.05  # Defensive range
    }
...
def get_team_totals_hr(batting_df):
    """Fetch total home runs from the 'Team Totals' row."""
    team_totals = batting_df[batting_df['Name'] == 'Team Totals']
    if not team_totals.empty:
        return int(team_totals['HR'].values[0])
    return 0

def calculate_team_x_hr(total_hr, games_played):
    """
    Calculate the X-HR grid based on the total home runs and number of games played.
    ...
    Returns:
    dict: X-HR grid where keys are dice ranges and values are the expected HRs for that range.
    """

    # Step 1: Subtract the expected power bonus HRs (54 for 162 games, 51 for 154 games)
    if games_played == 162:
        hr_bonus = 54
    ...
    # Calculate the remaining HRs after power bonus
    remaining_hr = total_hr - hr_bonus

    # Step 2: Calculate the proportional scaling factor
    scaled_hr = round((remaining_hr / games_played) * 216)

    # Step 3: Allocate the scaled HRs to the X-HR grid
    hr_distribution = {
        "111-125": min(scaled_hr, 33),
        "126-241": min(max(scaled_hr - 33, 0), 88),
        "242-261": min(max(scaled_hr - 121, 0), 12),
        "262-666": max(216 - scaled_hr, 0),
    }

    return hr_distribution

def calculate_team_unearned_runs(total_unearned_runs, games_played):
    """
...
def calculate_extra_inning_value():
    pass
...
def create_player_json(player_row, ranker, starters_threshold, batting_df, fielding_df, team_bat_df, year):
    ...
    team = player_row['team']  # Retrieve the player's team
    ...
    # Ensure sec_position is 'None' as a string when it's NaN
    ...
    bb_pct = player_batting_stats.get('BB%', 0)  # Remove the comma
    k_pct = player_batting_stats.get('K%', 0)    # Remove the comma
    sb = player_batting_stats.get('SB', 0)
    
    # Extract fielding stats for the player
    rtot_yr = fielder_row.get('rtot_yr', 0)  # Defensive Runs Saved per year

    # Speed rating calculation (different logic for pre-1933 seasons)
    if year < 1933:
        ...
        speed_data = {
            ...
            'PO': fielder_row.get('PO', 0),  # Putouts
            'A': fielder_row.get('A', 0),    # Assists
            'G': fielder_row.get('G', 0),    # Games played
            'position': primary_position
        }
        speed = calculate_speed_score(speed_data, year)

    # Extract team HRs from TEAM_BAT sheet (match by 'team' and 'year')
    team_stats = team_bat_df[(team_bat_df['team'] == team) & (team_bat_df['year'] == year)].squeeze()
    team_hr = team_stats.get('HR', 0)  # Total team HRs
    games_played = team_stats.get('G', 162)  # Get games played by the team
    ...
    return {
        ...
        "role": role,
        "position": primary_position,
        "sec_position": secondary_position,
        "batting": calculate_bat_rating(avg),
        "power": calculate_power_rating(slg),
        "eye": calculate_eye_rating(bb_pct, k_pct),  # Using bb_pct and k_pct
        "speed": speed,
        "fielding": calculate_fielding_rating(rtot_yr),
        "x_hr": calculate_player_x_hr(player_hr, team_hr, ranker, games_played),
        "extra_innings_range": 0,  # Placeholder until extra innings system is defined
        "injury": calculate_injury_rating(),
        "morale": 0,  # Placeholder until morale system is defined
        "popularity": 0,  # Placeholder for popularity if needed
        "salary": salary,
    }
...
def create_team_json(team_id, city, name, ballpark_name, stadium_value, market_size, team_x_hr, team_unearned_runs, home_field_advantage, players):
    """
    ...
    team_unearned_runs (dict): The unearned runs chart for the team.
    ...
    return {
        ...
        "team_unearned_runs": team_unearned_runs,  # Unearned runs chart
...
def process_team_roster(excel_file, save_directory):
    ...
    for _, manager_row in manager_df.iterrows():
        ...
        team_id = f"{team_abbr}_{team_info['year']}"
        city = team_info['city']
        ...
        team_unearned_runs = calculate_team_unearned_runs(total_unearned_runs, games_played)
        print(f"Team: {team_abbr}, Total Unearned Runs: {total_unearned_runs}, Games Played: {games_played}, Unearned Runs per Game: {team_unearned_runs}")
        ...
        team_json = create_team_json(
            ...
            market_size=market_size,
            team_x_hr=team_x_hr,
            team_unearned_runs=team_unearned_runs,
        ...
        json_filename = os.path.join(save_directory, f"{team_id}.json")
